...
public final class CameraHelper {
    ...
    public static boolean hasCamera2(Context context, boolean isUseFaceDetection) {
        if (context == null) return false;
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) return false;
        else if (Build.VERSION.SDK_INT > Build.VERSION_CODES.N) return true;
        try {
    ...
    public static File getOutputMediaFile(Context context, @CameraConfiguration.MediaAction int mediaAction) {
        ...
        File mediaStorageDir = new File(ChareemCamera.filePath);
        ...
        String timeStamp = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss", Locale.ENGLISH)
                .format(new Date());
        if (ChareemCamera.fileNames != null) timeStamp = ChareemCamera.fileNames;
        File mediaFile;
        if (mediaAction == CameraConfiguration.MEDIA_ACTION_PHOTO) {
            /*mediaFile = new File(mediaStorageDir.getPath() + File.separator +
                    "IMG_" + timeStamp + ".jpg");*/
            mediaFile = new File(ChareemCamera.filePath + timeStamp + ".jpg");
        } else if (mediaAction == CameraConfiguration.MEDIA_ACTION_VIDEO) {
            /*mediaFile = new File(mediaStorageDir.getPath() + File.separator +
                    "VID_" + timeStamp + ".mp4");*/
            mediaFile = new File(ChareemCamera.filePath + timeStamp + ".mp4");
        } else {
            return null;
        }
