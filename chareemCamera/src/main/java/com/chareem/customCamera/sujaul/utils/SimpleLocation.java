...
public class SimpleLocation {
    ...
    public void getSingleUpdate() {
        ...
        Log.d("SimpleLocation", "Get Single Update");
        mLocationListener = createLocationListener();
    ...
    public static void openDialogSetting(Activity act){
       /* LocationRequest mLocationRequest = new LocationRequest();
        ...
        SettingsClient settingsClient = LocationServices.getSettingsClient(act.getBaseContext());
        Task<LocationSettingsResponse> result = settingsClient.checkLocationSettings(locationSettingsRequest);
        result.addOnFailureListener(act, new OnFailureListener() {
            @Override
                ...
                switch (statusCode) {
                    case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                        try {
                            ...
                            rae.startResolutionForResult(act, REQUEST_CHECK_SETTINGS);
                        } catch (IntentSender.SendIntentException sie) {
                            Log.d("OpenDialogLocation", "Catch");
                        }
                        break;
                    case LocationSettingsStatusCodes.SETTINGS_CHANGE_UNAVAILABLE:
                        openSettings(act.getBaseContext());
                }
